...
REGION="/data/home/zuoxy/resource/DNASeq/hg19/Captured-regions_Agilent_HG19_AllExonV5.bed" #only include variants in these regions for mutsig analysis
##############################################
# 1. annotate VCF by using annovar
##############################################
pass_dir="./1-pass_mutect2"
mutsig_dir="./3-mutsigcv_annotate"
mutsig_temp_dir="./3-mutsigcv_annotate/temp"
annovar_dir="./2-annovar_output"
COMBINE_MAF="$mutsig_dir/combine_annovar.maf"
...
## CMD="java -jar $HOME/apps/GenomeAnalysisTK/3.4.0/GenomeAnalysisTK.jar -T SelectVariants -R $HOME/resource/DNASeq/hg19/ucsc_hg19.fa -V $INPUT_VCF -o $PASS_VCF -ef "
  ...
  then
    ...
    #perl $prepare_maf_script_pl $sample > $COMBINE_MAF
    HAS_HEADER="yes"
  else
    #perl $prepare_maf_script_pl $sample|perl -lane 'print if $. >1' >> $COMBINE_MAF
  fi
done
